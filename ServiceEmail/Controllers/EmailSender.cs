...
namespace ServiceEmail.UI.Controllers
{
    public class EmailSender : IJob
    {
        ...
        {
            using (MemoryStream stream = new MemoryStream(Encoding.ASCII.GetBytes(Helper.user.taskInfo.Last().DataOfTask)))
            {
                MailAddress from = new MailAddress("arhangel205@mail.ru", "Tom");
                // кому отправляем
                MailAddress to = new MailAddress(Helper.user.Email);
